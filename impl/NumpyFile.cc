...
NumpyTree::NumpyTree() : m_nb_elements(0)
{
...
void NumpyTree::fill_maps(std::type_index t_index, const std::string& s)
{
...
void NumpyTree::register_variable(const std::string& name, const void *p, const size_t size, std::string numpy_format, std::type_index t_index)
{

  NumpyData d(p, size, name, std::move(numpy_format), t_index);
  ...
  for (auto&& d_ : m_vector_of_pointer_to_data) // access by const reference
  {
      if(d_.name() == name)
      {
        ...
        s += "' already used !";
        cerr << s << endl;
        throw KeyAlreadyExistsException( s );
      }
  }
  m_vector_of_pointer_to_data.push_back(d);
}
//
void NumpyTree::register_variable(const std::string &name, const char *p, size_t nb_char)
{
...
//
//
void NumpyTree::register_variable(const std::string &name, const std::string *p, size_t nb_char)
{
  if(!nb_char)
    throw std::out_of_range("nb_char == 0 does not make any sense");
...
void OutputNumpyTreeFile::write_header()
{
...
void OutputNumpyTreeFile::close()
{
...
void OutputNumpyTreeFile::open(const std::string& s)
{
...
void InputNumpyTreeFile::open(const std::string &s)
{
...
void OutputNumpyTreeFile::write_variable(const std::string &name, const void *p, std::type_index t_index)
{
  this->register_variable(name, p, t_index);
}

void OutputNumpyTreeFile::write_variable(const std::string &name, const std::string *p, size_t nb_char)
{
  this->register_variable(name, p, nb_char);
}

void OutputNumpyTreeFile::write_variable(const std::string &name, const char *p, size_t nb_char)
{
  this->register_variable(name, p, nb_char);
}

OutputNumpyTreeFile::OutputNumpyTreeFile() : m_write_header_called(false)
{}
...
//
void NumpyTree::register_variable(const std::string &name, const void *p, std::type_index t_index)
{
...
bool OutputNumpyTreeFile::is_open()
{
...
bool InputNumpyTreeFile::is_open()
{
  return m_file.is_open();
}

uint64_t NumpyTree::nb_elements()
{
...
void InputNumpyTreeFile::read_header()
{
...
void InputNumpyTreeFile::read_next_entrie()
{
...
void InputNumpyTreeFile::read_variable(const std::string &name, void *p, std::type_index t_index)
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
    ...
    if(name == d.name())
    {
      if(t_index == typeid(string))
      {
        ...
        if(d.m_type_index != typeid(string))
          throw TypeMismatchHeaderException("Provided a string for non string variable");
      ...
      {
        if(d.m_type_index != typeid(string))
          throw TypeMismatchHeaderException("Provided a char* for non string variable");
        d.m_type_index_read = t_index;
        d.m_pointer_to_data = p;
        return;
      }
...
InputNumpyTreeFile::InputNumpyTreeFile() : m_read_header_called(false)
{}

void InputNumpyTreeFile::read_variable(const std::string &name, char *p)
{
  read_variable(name, p, typeid(char*));
}

bool InputNumpyTreeFile::data_to_read()
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
...
void InputNumpyTreeFile::read_variable(const std::string &name, std::string *p)
{
    read_variable(name, p, typeid(string));
}


bool InputNumpyTreeFile::has_variable(const std::string &name)
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
  for (auto&& d : m_vector_of_pointer_to_data) {
...
uint64_t InputNumpyTreeFile::nb_elements()
{
  return NumpyTree::nb_elements();
}

void InputNumpyTreeFile::read_entrie(const uint64_t &i)
{
...
type_index InputNumpyTreeFile::get_type_of_variable(const std::string &name)
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
  for (auto&& d : m_vector_of_pointer_to_data) {
...
bool OutputNumpyTreeFile::s_registered =  OutputTreeFileFactory::_register(OutputNumpyTreeFile::_get_factory_name(), &OutputNumpyTreeFile::_create_method<OutputNumpyTreeFile>);
bool InputNumpyTreeFile::s_registered =  InputTreeFileFactory::_register(InputNumpyTreeFile::_get_factory_name(), &InputNumpyTreeFile::_create_method<InputNumpyTreeFile>);
