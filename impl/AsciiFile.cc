...
void AsciiTree::register_variable(const std::string &name, const void *p, std::type_index t_index)
{
...
void AsciiTree::register_variable(const std::string &name, const char *p, size_t )
{
    this->register_variable(name, p, typeid(char*));
}

void AsciiTree::register_variable(const std::string &name, const std::string *p, size_t )
{
...
template<>
void read_from_string<char*>(AsciiData *d, const std::string& s)
{
...
void OutputAsciiTreeFile::write_variable(const std::string &name, const void *p, std::type_index t_index)
{
...
void InputAsciiTreeFile::read_next_entrie()
{
    string line;
...
void InputAsciiTreeFile::read_variable(const std::string &name, std::string *p)
{
  this->read_variable(name, p, typeid(string));
}

void InputAsciiTreeFile::read_variable(const std::string &name, char *p)
{
  this->read_variable(name, p, typeid(char*));
}

void InputAsciiTreeFile::read_variable(const std::string &name, char *p, size_t nb_char)
{
...
InputAsciiTreeFile::InputAsciiTreeFile():
...
bool InputAsciiTreeFile::has_variable(const std::string &name)
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
...
type_index InputAsciiTreeFile::get_type_of_variable(const std::string &name)
{
  if(!m_read_header_called)
    throw std::logic_error("read_header not called");
//  return typeid(nullptr);
  throw NoTypeInHeaderException("");
}

uint64_t InputAsciiTreeFile::nb_elements()
{
  return m_number_of_lines_in_file;
}

void InputAsciiTreeFile::read_entrie(const uint64_t &i)
{
...
bool OutputAsciiTreeFile::s_registered =  OutputTreeFileFactory::_register(OutputAsciiTreeFile::_get_factory_name(), &OutputAsciiTreeFile::_create_method<OutputAsciiTreeFile>);
bool InputAsciiTreeFile::s_registered =  InputTreeFileFactory::_register(InputAsciiTreeFile::_get_factory_name(), &InputAsciiTreeFile::_create_method<InputAsciiTreeFile>);
